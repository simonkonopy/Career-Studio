...
3. **Review AI suggestions.** With explicit consent and a configured server key, the interviewer asks follow-up questions and proposes supported profile changes. The customer accepts or rejects each proposal. Accepting a suggestion makes the profile require review again.
...
The September 9, 2026 handoff suite passes 83 tests: 70 unit/server/document/feed/backup tests and 13 browser journeys, including the account-free demo, arbitrary skill interviews, and custom task persistence. Rerun it in the receiving environment and keep the resulting CI run with the release. The GitHub workflow installs Chromium with Linux dependencies and builds the Docker image. Test counts establish tested behavior, not security certification, model truthfulness, hosting readiness, or customer outcomes.
...
Node server.js
  ...
  |-- lib/resume-model.js  profile model, import heuristics, task catalog, resume text
...
- Keep manual editing and existing records usable when AI is unavailable or quotas are exhausted.
...
| `POST /api/interview`, `POST /api/proposals/:id/accept`, `POST /api/proposals/:id/reject` | Interview and owner-approved suggestions |
...
Uploaded file bytes are parsed in bounded server workers without writing the raw file to disk. Extracted text can become part of the stored profile. With customer consent, selected profile/interview/job content is sent to OpenAI. The adapter uses `store: false` and best-effort contact-field redaction; these settings do not constitute complete anonymization or a promise of zero provider retention.
...
The supported promise is: **understand your actual skills, turn them into reviewed resumes, evaluate job requirements clearly, and keep your search organized.** Show one complete journey in the fictional demo, then explain that the real product stores the customer's approved work and uses operator-provided AI after consent.
