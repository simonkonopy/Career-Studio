...
**Two implemented AI modes:** the hosted SaaS path uses the operator's OpenAI API account; customers do not need a subscription or key. An **experimental local companion** lets one local Career Studio account connect eligible ChatGPT access through the official Codex-managed browser sign-in. Local mode is pinned to Codex 0.153.4, rejects hosted/production configuration, and never falls back to the operator's API key. A ChatGPT plugin remains a future implementation. Paid plans and checkout are deferred until pilot costs and customer outcomes are measured.

This directory is designed to be the root of a **dedicated product repository**. It does not read the personal job-search dashboard, applications, resumes, or research in the parent project. Copy only the source files described below; do not copy the parent repository or its history.
...
For the experimental customer-subscription preview on the customer's own computer, use `npm run start:companion` with `LOCAL_COMPANION=1`, then open **Your account → Connect ChatGPT**. Sign-in and permission to share career context are separate actions. The companion uses in-memory authentication and ephemeral threads in a dedicated runtime directory; temporary metadata still touches disk and is cleaned up on termination. It does not reuse existing Codex credentials. Follow [Local companion setup and limits](docs/LOCAL-COMPANION.md). Runtime startup and protocol compatibility have been checked without signing in; live subscription inference still needs a consenting-user pilot.
...
- Optional local ChatGPT connection owned by the authenticated Career Studio account, with official managed sign-in, version checks, private child-process transport, restricted capabilities, cancellation, disconnect, and no API fallback. Both Studio allowances and the customer's actual ChatGPT limits apply.
...
The next engineer should follow [Engineering handoff](docs/ENGINEERING_HANDOFF.md), [SaaS business and release plan](docs/SAAS_BUSINESS_PLAN.md), and [Security instructions](docs/SECURITY.md). [Customer subscription delivery](docs/CUSTOMER_SUBSCRIPTION_DELIVERY.md) distinguishes the implemented [experimental local companion](docs/LOCAL-COMPANION.md) from the future ChatGPT plugin. The written guides supersede the deck's older delivery-status descriptions; the Fable-themed deck remains the visual product overview. The product owner can use the [Sales playbook](docs/SALES_PLAYBOOK.md) for the pitch, five-minute demo, customer discovery, pilot measurements, and accurate answers to objections. The presentation contains approved fictional demo images; exclude unscreened screenshots and customer content from the transfer.
...
Copy `HANDOFF.md`, `server.js`, `lib/`, `public/`, `scripts/`, `tests/`, `docs/`, `package.json`, `package-lock.json`, `.env.example`, `.gitignore`, `.dockerignore`, `Dockerfile`, `.github/`, and this README into a fresh directory. Review the files for personal data before committing them to the dedicated product repository. Keep real customer data, `.env`, `data/`, `backups/`, `node_modules/`, browser traces, test output, companion runtime directories, and unscreened screenshots out of that transfer.
