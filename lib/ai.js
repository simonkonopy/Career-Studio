...
function createAI({ apiKey, model = 'gpt-5.6-terra', fetchImpl = globalThis.fetch, timeoutMs = 45000 } = {}) {
  const enabled = typeof apiKey === 'string' && apiKey.trim().length > 0;
  if (typeof fetchImpl !== 'function' || typeof model !== 'string' || !/^[a-zA-Z0-9._:-]{1,100}$/.test(model) || !Number.isFinite(timeoutMs) || timeoutMs < 1 || timeoutMs > 120000) throw inputError('The AI service configuration is invalid.');
  ...
  async function request(kind, payload, instructions, validate, signal) {
    ...
    const work = async () => {
      ...
      let result;
      try { result = JSON.parse(outputs[0].text); } catch { throw invalid(); }
